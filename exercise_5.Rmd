...
grouped_wildschwein2 <- joined_wildschwein%>%
  ...
  #Preparing data so it's more tidy
  mutate(Frucht = ifelse(is.na(Frucht)==TRUE, "Andere", Frucht), newFrucht = ifelse(nrel >= 10, Frucht, "Andere"))
...
ggplot(grouped_wildschwein2)+
  ...
  labs(title = "Aufenthaltsorte von Wildschweinen über den Tag (Relativ)")+
  ylab("[%]")+
  xlab("Stunde")
...
ggplot(grouped_wildschwein2)+
  ...
  facet_wrap(~TierName, nrow = 1)+
  labs(title = "Aufenthaltsorte von Wildschweinen über den Tag (Total)")+
  ylab("Anzahl Fixes")+
  xlab("Stunde")
